...
class HealthViewsUnitTests(unittest.TestCase):
    ...
    @mock.patch(
        "apps.common.views._db_check", return_value={"status": "ok", "latency_ms": 1.23}
    )
    ...
    @mock.patch(
        "apps.common.views._redis_ping",
        return_value={"status": "fail", "error": "unreachable"},
    )
    @mock.patch(
        "apps.common.views._db_check",
        return_value={"status": "fail", "error": "db down"},
    )
    def test_ready_health_degraded_on_dependency_failure(
        self, mock_db_check, mock_redis_ping, _mock_getenv
    ):
        response = views.ready_health(None)
