...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        self.log.info('Processing registration request', username=serializer.validated_data.get('username'))
        result = self.service.register(serializer.validated_data)
        if isinstance(result, tuple):
            code, message, details = result
            self.log.warning('Registration failed', code=code, detail=message, details=details)
...
class LogoutView(APIView):
    ...
    def post(self, request):
        try:
            ...
            self.log.exception('Logout failed', user_id=getattr(request.user, 'id', None))
    ...
    def post(self, request):
        ...
        for token in tokens:
            try:
                ...
                self.log.exception('Failed to blacklist token during logout-all', user_id=user_id, token_id=token_id)
        self.log.info('User logged out from all devices', user_id=user_id, tokens_invalidated=count)
