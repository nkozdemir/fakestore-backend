...
class UserAddressDetailView(APIView):
    ...
    def put(self, request, address_id: int):
        ...
        if not user_id:
            self.log.warning(
                "Unauthorized address replace attempt", address_id=address_id
            )
            return error_response("UNAUTHORIZED", "Authentication required")
        serializer = AddressWriteSerializer(data=request.data)
        try:
        ...
        dto = self.service.update_user_address(
            user_id, address_id, serializer.validated_data
        )
        if not dto:
            self.log.warning(
    ...
    def patch(self, request, address_id: int):
        ...
        if not user_id:
            self.log.warning(
                "Unauthorized address patch attempt", address_id=address_id
            )
            return error_response("UNAUTHORIZED", "Authentication required")
    ...
    def delete(self, request, address_id: int):
        ...
        if not user_id:
            self.log.warning(
                "Unauthorized address delete attempt", address_id=address_id
            )
            return error_response("UNAUTHORIZED", "Authentication required")
