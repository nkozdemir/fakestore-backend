...
class CartService:
    ...
    def create_cart(self, user_id: int, data: Dict[str, Any]):
        ...
        with transaction.atomic():
            ...
            for item in command.items:
                ...
                if not product:
                    self.logger.warning('Skipping missing product during cart creation', product_id=item.product_id)
                    continue
    ...
    def update_cart(self, cart_id: int, data: Dict[str, Any], user_id: Optional[int] = None):
        ...
        if not cart:
            self.logger.warning('Cart update failed: not found', cart_id=cart_id, user_id=user_id)
            return None
    ...
    def delete_cart(self, cart_id: int, user_id: Optional[int] = None) -> bool:
        ...
        if not cart:
            self.logger.warning('Cart deletion failed: not found', cart_id=cart_id, user_id=user_id)
            return False
        with transaction.atomic():
    ...
    def patch_operations(self, cart_id: int, ops: Dict[str, Any], user_id: Optional[int] = None):
        ...
        cart: Optional[Cart] = self.carts.get(**filters)
        if not cart:
            self.logger.warning('Cart patch failed: not found', cart_id=cart_id, user_id=user_id)
            return None
    ...
    def _rebuild_items(self, cart: Cart, raw_items: List[Dict[str, Any]]):
        ...
        for raw in raw_items:
            ...
            if not product:
                self.logger.warning('Skipping missing product during cart rebuild', cart_id=cart.id, product_id=command.product_id)
                continue
    ...
    def _apply_add_ops(self, cart: Cart, existing_map: Dict[int, CartProduct], add_ops: List[CartItemCommand]):
        for item in add_ops:
            product = self.products.get(id=item.product_id)
            if not product:
                self.logger.warning('Skipping missing product in add op', cart_id=cart.id, product_id=item.product_id)
                continue
    ...
    def _apply_update_ops(self, cart: Cart, update_ops: List[CartItemCommand]):
        for item in update_ops:
            product = self.products.get(id=item.product_id)
            if not product:
                self.logger.warning('Skipping missing product in update op', cart_id=cart.id, product_id=item.product_id)
                continue
