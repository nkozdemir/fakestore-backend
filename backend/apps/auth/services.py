...
class RegistrationService:
    ...
    def _check_uniqueness(self, username: str, email: str) -> Optional[tuple]:
        if self.users.username_exists(username):
            self.logger.info('Registration rejected: username already exists', username=username)
    ...
    def register(self, data: Dict[str, Any]):
        username = data['username'].strip()
        email = data['email'].strip()
        self.logger.debug('Received registration request', username=username, email=email)
        ...
        self.logger.info('User registered successfully', user_id=user.id, username=user.username)
        return {
            'id': user.id,
