...
if settings.DEBUG:
    ...
    def _static_schema_response(format: str):  # pragma: no cover (simple IO)
        rel = settings.OPENAPI_STATIC_JSON if format == 'json' else settings.OPENAPI_STATIC_YAML
        static_base = Path(settings.BASE_DIR) / 'static' / 'schema'
        file_path = static_base / ('openapi.json' if format == 'json' else 'openapi.yaml')
    ...
    urlpatterns += [
        ...
        path('schema.yaml', lambda r: _static_schema_response('yaml'), name='schema-yaml'),
