...
class ProductService:
    ...
    def list_products(self, category: Optional[str] = None):
        self.logger.debug('Listing products', category=category, cache_enabled=not self.disable_cache)
    ...
    def update_product(self, product_id: int, data: Union[Dict[str, Any], ProductUpdateCommand], partial: bool = False):
        ...
        if not product:
            self.logger.warning('Product update failed: not found', product_id=product_id)
            return None
    ...
    def delete_product(self, product_id: int) -> bool:
        ...
        if not product:
            self.logger.warning('Product deletion failed: not found', product_id=product_id)
            return False
    ...
    def get_rating_summary(self, product_id: int, user_id: Optional[int] = None):
        product = self.products.get(id=product_id)
        if not product:
            self.logger.info('Rating summary requested for missing product', product_id=product_id, user_id=user_id)
            return None
        user_rating = None
        if user_id:
            ...
            if r:
                user_rating = r.value
        self.logger.debug('Returning rating summary', product_id=product_id, user_id=user_id)
        return {
    ...
    def set_user_rating(self, product_id: int, user_id: int, value: Union[int, RatingSetCommand]):
        ...
        if cmd.value < 0 or cmd.value > 5:
            self.logger.warning('Rejecting rating outside bounds', product_id=product_id, user_id=user_id, value=cmd.value)
            return ('VALIDATION_ERROR', 'value must be between 0 and 5', {'value': cmd.value})
        product = self.products.get(id=product_id)
        if not product:
            self.logger.warning('Rating failed: product not found', product_id=product_id, user_id=user_id)
        ...
        if existing:
            ...
            existing.save()
            self.logger.info('Updated product rating', product_id=product_id, user_id=user_id, value=cmd.value)
        else:
            self.ratings.create(product_id=product_id, user_id=user_id, value=cmd.value)
            self.logger.info('Created product rating', product_id=product_id, user_id=user_id, value=cmd.value)
    ...
    def delete_user_rating(self, product_id: int, user_id: int):
        product = self.products.get(id=product_id)
        if not product:
            self.logger.warning('Rating delete failed: product not found', product_id=product_id, user_id=user_id)
            return ('NOT_FOUND', 'Product not found', {'id': product_id})
        existing = self.ratings.for_product_user(product_id, user_id)
        if not existing:
            # Idempotent delete - still return summary
            self.logger.info('No rating to delete; returning summary', product_id=product_id, user_id=user_id)
            return self.get_rating_summary(product_id, user_id)
        existing.delete()
...
class CategoryService:
    ...
    def update_category(self, category_id: int, data: Dict[str, Any]):
        ...
        if not category:
            self.logger.warning('Category update failed: not found', category_id=category_id)
            return None
    ...
    def delete_category(self, category_id: int) -> bool:
        ...
        if not category:
            self.logger.warning('Category deletion failed: not found', category_id=category_id)
            return False
